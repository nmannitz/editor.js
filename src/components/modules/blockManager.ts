...
export default class BlockManager extends Module {
   ...
   * @param {string} options.id - sort id
   *
   * @returns {Block}
   */
  public composeBlock({ tool, data = {}, id = '' }: {tool: string; data?: BlockToolData, id: string}): Block {
  ...
  public insertInitialBlockAtIndex(index: number, needToFocus = false): Block {
    const block = this.composeBlock({ tool: this.config.initialBlock, id: mixin.createId() });
